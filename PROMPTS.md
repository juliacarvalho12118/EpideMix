...
> Crie um código completo em Python que simule a propagação das principais doenças infectocontagiosas em uma população distribuída em um grid bidimensional. O modelo deve incluir estados de saúde (susceptível, infectado, recuperado), parâmetros configuráveis (taxa de infecção, taxa de recuperação, tamanho da população, número inicial de infectados) e evolução temporal. A visualização deve ser feita em tempo real usando matplotlib ou biblioteca equivalente, mostrando um mapa de calor que represente a densidade de infectados em cada região do grid. O código deve ser auto-contido, com funções bem estruturadas e comentários explicativos, pronto para ser executado sem necessidade de ajustes externos.
